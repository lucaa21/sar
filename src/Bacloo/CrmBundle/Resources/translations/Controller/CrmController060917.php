							...
							foreach($tags as $tag)//Pour chacun des tags
								{
									...
									//print_r($list_fichespot);
										if(!empty($listintfiche2))//si il y a des interresses dans la table interresses
										{
										//echo 'loooooooooooooooo';
											foreach($listintfiche2 as $lif)//Pour chaque utilisateur avec les tags de la fiche
											{
													...
													$fichecp = substr($fiche->getCp(), 0, 2);
													if($fichecp == 97)
													{
														$fichecpok = substr($fiche->getCp(), 0, 3);
													}
													else
													{
														$fichecpok = substr($fiche->getCp(), 0, 2);
													}
													$query = $em->createQuery(
														'SELECT u.cp
														FROM BaclooUserBundle:User u
													...
													if(!isset($fichecheck) && !isset($fichecheck2) && isset($cpcheck))
														{
															$today = date('Y-m-d');
															$em = $this->getDoctrine()->getManager();
															$query = $em->createQuery(
										...
										//si pas encore d'interresses dans la table interresses
											...
											// {
													...
													if(!isset($fichecheck) && !isset($fichecheck2) && isset($cpcheck))//si cette fiche n'a jamais été proposée
													{
...
//$critere = array('jmr', 'toto');
		...
		$user  = $em->getRepository('BaclooUserBundle:User')		
			...
			{//echo 'ici'.$v;
				...
				{//echo ' on est prospect';
					...
					if($session->get('id') > 0)
					{
						return $this->redirect($this->generateUrl('bacloocrm_find', array('id' => $session->get('id'), 'page' => $session->get('page'), 'view' => $session->get('view') )));
					}					
				}
			...
			if ($request->getMethod() == 'POST') {
			  $form->bind($request);
				  if ($form->isValid()) {
					...
					$id = $search->getid();
					$session->set('id', $id);
					return $this->redirect($this->generateUrl('bacloocrm_find', array('id' => $id, 'view' => $view )
					));
				}
			}
		...
		$modules  = $em->getRepository('BaclooCrmBundle:Modules')		
			...
			{
			if(!isset($lesfichesss)){$lesfichesss = 0;}
