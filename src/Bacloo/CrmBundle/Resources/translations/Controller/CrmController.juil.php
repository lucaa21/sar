...
//
				if(!empty($besoins))//Si la fiche a des besoins renseignés
				{//echo 'adada';
				$tagsfiche  = $em->getRepository('BaclooCrmBundle:Fiche')		
							   ->findOneById($id);				
...
// echo '  décollage';
	...
	// On récupère la fiche qui nous intéresse
				...
				// $originalalteruser = $em->getRepository('BaclooCrmBundle:Alteruser')		
				 // ->findByFicheid($id);
				 // print_r($fiche);
				if(empty($fiche) OR !isset($fiche)){echo 'palter2';return $this->redirect($this->generateUrl('bacloocrm_search', array()));
				}
...
// echo '$originalprospo->getRaisonSociale()'.$originalprospo->getRaisonSociale().'<------';
	...
	//echo 'ici';
				foreach ($transaction as $transbdd)//boucle pour recup transactions
				{
				  ...
				  {
	//echo 'la';
						//On récupère la transaction correspondant en BDD
						$em = $this->getDoctrine()->getManager();
						$query = $em->createQuery(
							'SELECT t
...
// echo $transform->getRaisonSociale();							
					...
					//On récupère toutes les notes des transactions du vendeur
					$em = $this->getDoctrine()
							   ->getManager()
...
//Recherche de Prospects	
 	public function searchbaclooAction($mode)
		...
		$uid = $query->getSingleScalarResult();		
			...
			// Si des prospectsbacloo lui ont deja été proposés on compare aux nouveaux avant d'ajouter
				$em = $this->getDoctrine()->getManager();
...
// le paramètre de l'action doit se remplir en fonction des critères de recherche		
 	public function findbaclooAction($id, $mode, $page, $toc, Request $request)
...
//echo 'iiiiiiiiiiiiiiii'.$i;
 	...
 	public function showbacloolistAction($id, $mode, $page, $toc, Request $request)
	{//echo 'le show';
	$usersess = $this->get('security.context')->getToken()->getUsername(); if(empty($usersess) or !isset($usersess) or $usersess == 'anon.'){return $this->redirect($this->generateUrl('fos_user_security_login'));}//Récupère le nom d'utilisateur
	//echo 'la page'.$page;
	$nbparpage = 20;
			...
			{//echo '8'.$pr->getRaisonSociale();
				...
				if(isset($prospo2) && !empty($prospo2) && $prospo2->getRaiSonsociale() == $pr->getRaisonSociale())
				{
					if($this->getRequest()->request->get('pagination') == 'Tout cocher')
					{
						...
						{
						$prospo2->setAcheter('1');//echo 'toc=1';
						}
					}
					elseif($this->getRequest()->request->get('pagination') == 'Tout décocher')
					{
						if(isset($prospo2) && !empty($prospo2))
						{
						$prospo2->setAcheter('0');//echo 'toc=2';
						}											
					}
					else
					{
						if(isset($prospo2) && !empty($prospo2) && $prospo2->getRaiSonsociale() == $pr->getRaisonSociale())
						{
						$prospo2->setAcheter($pr->getAcheter());//echo 'toc=null';
						}											
					}
					// $em->remove($pr);
					// $em->persist($pr);
...
$nbresultats = count($fiche);//echo 'nbresult'.$nbresultats;
	...
	public function showfavorisAction()
		{
			$usersess = $this->get('security.context')->getToken()->getUsername(); if(empty($usersess) or !isset($usersess) or $usersess == 'anon.'){return $this->redirect($this->generateUrl('fos_user_security_login'));}//Récupère le nom d'utilisateur	
					...
					//On récupère les favoris du user connecté
					$em2 = $this->getDoctrine()
							   ->getManager()
							   ->getRepository('BaclooCrmBundle:Favoris');
